...
것을 주제별로 나눴다. 코드를 건드리기 전에 먼저 볼 요약은 프로젝트 루트의 `CLAUDE.md`를 참고한다.
...
> 설계 기록으로만 남아있다. 지금 실제로 하는 일은 `CLAUDE.md`를 기준으로 본다.
>
> **추가로(2026-08-12)**: 남은 검색·추천 기능도 실제로는 동작하지 않는다 — 쿠팡은 검색 페이지까지
> Akamai로 막고, 쿠팡·네이버 공식 API도 개인 개발자에게는 상품 검색을 열어주지 않는다. 자세한 내용은
> `CLAUDE.md`의 "왜 상품 검색도 API로 자동화돼 있지 않은가" 참고.
>
> **추가로(2026-08-21)**: 위 막힘을 근거로 라이브 연동을 포기하고, 수동으로 저장한 상품 스냅샷으로
> 에이전트 판단 로직을 시연하는 쪽으로 방향을 정했다. [snapshot-mode.md](snapshot-mode.md) 참고.
...
- [snapshot-mode.md](snapshot-mode.md) — 라이브 연동 포기, 수동 스냅샷 기반 데모로 전환한 결정과 계획
- [harness.md](harness.md) — `.claude/` 개발 하네스 구성: 왜 이렇게/왜 나머지는 안 만들었는지
